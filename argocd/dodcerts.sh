...
# Request sudo access at the beginning of the script
if ! sudo -v; then
  echo "This script requires sudo access. Exiting."
  exit 1
fi

# Keep the sudo session active
while true; do
  sudo -v
  sleep 60
  kill -0 "$$" || exit
...
#   echo "Downloading bundle from $url$bundle"
  ...
  # Extract the bundle
#   cd $certdir
#   wget -qP /tmp/certs $bundle
#   unzip -qj /tmp/certs/${bundle##*/} -d /tmp/certs

  # Check for existence of PEM format p7b.
  if [ -f "/tmp/certs/*_dod_pem.p7b" ]; then
  ...
  openssl pkcs7 -inform ${certform} -print_certs -in /tmp/certs/${certfile} |
...
#   rm -fr /tmp/certs
  echo "Adding certificates to $certdir"
  sudo $update
}
